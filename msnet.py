...
class nnn_net(nn.Module):
    ...
    def forward(self, nnn):
        ...
        dec5 = self.dec5(F.max_pool2d(nnn4, kernel_size=2, stride=2))
...
class MSNet(nn.Module):
    ...
    def forward(self, rgbnnd):
        # Assert input has 4 channels
        assert rgbnnd.shape[1] == 4, f"Expected input with 4 channels, but got {rgbnnd.shape[1]}"
